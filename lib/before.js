...
module.exports = function (scope, cb) {
    ...
    var requiredAttributes = _.pickBy(model.attributes, function (attribute) {
        return attribute.required && attribute.model !== 'organization';
    });
    ...
    scope.generateRequiredAttributes = function (requiredAttributes) {
        var array = _.map(requiredAttributes, '__name');
        array.push('createdAt');
        array.push('updatedAt');

        return JSON.stringify(array, null, '\t');
    };


    scope.modelAttributes = _.filter(requiredAttributes, function (attribute) {
        return attribute.model && (attribute.model != 'organization');
    });
